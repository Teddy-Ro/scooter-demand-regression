...
1. Запустить `weather_preprocessing.ipynb`, чтобы получить очищенный файл погоды.​
2. Запустить `rides_preprocessing.ipynb`, чтобы собрать почасовой датасет с признаками.​
3. Запустить `Regression-modeling.ipynb` для обучения и сохранения модели в `models/scooter_demand_model.pkl`.​
